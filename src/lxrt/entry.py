...
class LXRTEncoder(nn.Module):
    ...
    def forward(self, sents, feats, visual_attention_mask=None):
        ...
        input_ids = torch.tensor([f.input_ids for f in train_features], dtype=torch.long).cuda(device=feats[0].device)
        input_mask = torch.tensor([f.input_mask for f in train_features], dtype=torch.long).cuda(device=feats[0].device)
        segment_ids = torch.tensor([f.segment_ids for f in train_features], dtype=torch.long).cuda(device=feats[0].device)
